...
See `?Dialyzer`{.R} for explanation of the variables (data comes with the package nlme).

The data highlights the flexibility of gls for structured (1|subject) temporal data. Unfortunately, nlme does not interface with DHARMa.
...
You can download the data set at the link above and run the following code (don't forget to adjust the corresponding path):

```{r chunk_chapter6_chunk8, echo=TRUE, eval=FALSE, purl=FALSE}
anolisTree = read.tree("/tmp/anolis.phy")
anolisData = read.csv("/tmp/anolisDataAppended.csv", row.names = 1)
save(anolisTree, anolisData, file = "/tmp/anolis.Rdata")
```

Alternatively, download the data set at <a href="https://www.dropbox.com/s/8akya9qpg4oln61/anolis.Rdata?dl=0" target="_blank" rel="noopener">https://www.dropbox.com/s/8akya9qpg4oln61/anolis.Rdata?dl=0</a> and read it in (don't forget to adjust the corresponding path).

```{r chunk_chapter6_chunk9, echo=TRUE, eval=FALSE, purl=FALSE}
load("/tmp/anolis.Rdata")
```

Perform analysis:

```{r chunk_chapter6_chunk10, echo=TRUE, eval=TRUE}
library(ape)
library(geiger)
library(nlme)
library(phytools)
...
from package EcoData, and check for spatial dependencies in the residuals. See data set help for details on the variables. 
...
```{r chunk_chapter6_chunk15, echo=FALSE, out.width="150%", out.height="150%"}
