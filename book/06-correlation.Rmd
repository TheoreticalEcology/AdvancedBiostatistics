...
As our first example, I look at the hurricane study from yesterday, which is, after all, temporal data. This data set is located in `EcoData`{.R}.
...
```{r chunk_chapter6_chunk10, echo=FALSE, eval=TRUE}
...
from package `EcoData`{.R}, and check for spatial dependencies in the residuals. See the data set's help for details on the variables.
...
```{r chunk_chapter6_chunk18, echo=TRUE, eval=FALSE, purl=FALSE}
library(lme4)
...
```{r chunk_chapter6_chunk19, echo=TRUE, eval=FALSE, purl=FALSE}
library(brms)
...
# Running time: 40 s compilation + 50 s sampling = 1.5 min.
```
