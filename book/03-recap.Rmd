...
Now we fitted a model. But does it even fit to the data? Let's take the slope only model `lm(Ozone ~ Wind - 1, data = airquality)`{.R}, where we assumed that the regression line should go through (0, 0). Maybe we have good reasons to think that this should be the case biologically, but our data seem to suggest a different behavior. So, the question is: do the model assumptions `lm(Ozone ~ Wind - 1)`{.R} even fit to our data? 
...
```{r chunk_chapter3_chunk17, echo=FALSE, eval=TRUE}
plot(Ozone ~ cTemp, data = airquality)
abline(fit)
```
...
```{r chunk_chapter3_chunk18, echo=FALSE, eval=TRUE}
library(effects)
...
```{r chunk_chapter3_chunk19, echo=FALSE, eval=TRUE}
fit = lm(Ozone ~ Wind + Temp, data = airquality)
plot(allEffects(fit, partial.residuals = T))
```

This looks already better, but there seems to be still a bit of a pattern regarding the scatter of the observed data around the regression line. We can get the difference between model and observations via `residuals(fit)`{.R}, and we could plot them against the model predictions (which can be obtained via the predict function) via

```{r chunk_chapter3_chunk20, echo=FALSE, eval=TRUE}
...
```{r chunk_chapter3_chunk21, echo=FALSE, eval=FALSE}
par(mfrow = c(2, 2))
```

which produces a figure with 2 x 2 = 4 panels.

```{r chunk_chapter3_chunk22, echo=FALSE, eval=TRUE}
par(mfrow = c(2, 2))
plot(fit)
```
...
library(MASS)
fit = lm(Ozone ~ Wind, data = airquality)
# Calculate optimal transformation for Ozone^lambda to achieve residuals as normally distributed as possible.
boxcox(fit)
```
