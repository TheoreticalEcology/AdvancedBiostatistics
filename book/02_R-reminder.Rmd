...
Exchanging LIBRARY with the name of the library you want to install. The default is to search the package in CRAN, but you can specify other repositories or file locations in the function. To save typing for you, you can install an entire list of libraries that you will need by exexcuting the following commands: 

```{r chunk_chapter2_0, code=xfun::read_utf8('dependencies.R'), eval=FALSE}
```

Moreover, in this book, we will often use data sets from the EcoData package, which is not on CRAN. To install the package, run:
...
```{r chunk_chapter2_12}
head(iris[iris$Species == "virginica", ], 20)   # Show first 20 entries only.
```

Note that this is identical to the following:

```{r chunk_chapter2_13}
head(subset(iris, Species == "virginica"), 20)  # Show first 20 entries only.
```
