...
Exchange "LIBRARY" with the name of the library you want to install. The default is to search the package in CRAN, but you can specify other repositories or file locations in the function. To save typing for you, you can install an entire list of libraries that you will need by executing the following commands: 

```{r chunk_chapter2_1, code=xfun::read_utf8('dependencies.R'), eval=FALSE}
```

Moreover, in this book, we will often use data sets from the `EcoData`{.R} package, which is not on CRAN. To install the package, run:
...
```{r chunk_chapter2_26, eval=knitr::is_html_output(excludes = "epub"), results = 'asis', echo = F}
cat(
  '<iframe width="560" height="315"
