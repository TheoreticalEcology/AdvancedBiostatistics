...
Exchange "LIBRARY" with the name of the library you want to install. The default is to search the package in CRAN, but you can specify other repositories or file locations in the function. 

In this book, we will often use data sets from the `EcoData`{.R} package, which is not on CRAN, but on a GitHub page. To install the package, run:
...
allow you to explore the structure of variables and the data they contain. 
...
Ask yourself, or discuss with your partner(s) - what is the meaning of the three functions, and what is the structure / properties of the following data types in R:
...
Q1: Atomic types: e.g. numeric, factor, boolean ...; List can have severa tyes, vector not! data.frame is list of vectors. matrix is 2-dim array, array can have any dim, only one type.

Q2: iris, like most simple datasets, is of type data.frame
...
```{r chunk_chapter2_13}
head(iris[iris$Species == "virginica", ])   # Show first 20 entries only.
```

Note that this is identical to using the subset command:

```{r chunk_chapter2_14}
head(subset(iris, Species == "virginica"))  # Show first 20 entries only.
```
