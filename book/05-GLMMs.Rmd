...
We want to look at the titanic data set. A cleaned version of this is in the package EcoData, which you have to install first from GitHub. If you haven't already installed this package, I hope this works for you:
...
We will talk about DHARMa more later, see also comments on testing binomial GLMs <a href="https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html#binomial-data" target="_blank" rel="noopener">here</a>.
...
Zero-inflated GLMMs can, for example, be fit with the glmmTMB package, using `ziformula = ~ 0`{.R}. 

***How to check for zero-inflation***

*Important*: Do not check for zero-inflation in the response. 

DHARMa has a function for testing zero-inflation:
...
The data is available <a href="https://elearning.uni-regensburg.de/pluginfile.php/2365101/mod_book/chapter/48142/Hurricanes.Rdata" target="_blank" rel="noopener">https://www.pnas.org/content/111/24/8782</a>.

You can adopt the file's path and use the following code to read in the data:

```{r chunk_chapter5_task_5, eval=FALSE, purl=FALSE, message=FALSE, warning=FALSE}
library(readxl)
Data = read_excel("~/Downloads/pnas.1402786111.sd01.xlsx", n_max = 92)
...
```{r chunk_chapter5_task_6, eval=FALSE, purl=FALSE, message=FALSE, warning=FALSE}
plot(Data$MasFem, Data$NDAM, cex = 0.5, pch = 5)
points(Data$MasFem, Data$NDAM, cex = Data$alldeaths/20, pch = 4, col= "red")
```

The original model from the paper fits a negative binomial, using mgcv.

```{r chunk_chapter5_task_7, eval=FALSE, purl=FALSE, message=FALSE, warning=FALSE}
originalModelGAM = gam(alldeaths ~ MasFem * (Minpressure_Updated.2014 + NDAM), 
    data = Data, family = nb, na.action = "na.fail")
...
Analyze the Marmot data. Available at <a href="http://highstat.com/Books/Book4/Marmots.txt" target="_blank" rel="noopener">http://highstat.com/Books/Book4/Marmots.txt</a>. You can read the URL directly with R, no need to download. A description of the data is available <a href="https://elearning.uni-regensburg.de/pluginfile.php/1922323/mod_assign/introattachment/0/Marmots.pdf?forcedownload=1" target="_blank" rel="noopener">https://osf.io/9yh4x/</a>.

Use the following snippet to read in the data:

```{r chunk_chapter5_task_10, eval=FALSE, purl=FALSE, message=FALSE, warning=FALSE}
marmots = read.table(url("http://highstat.com/Books/Book4/Marmots.txt"))
```
...
```{r chunk_chapter5_chunk23, echo=TRUE, eval=TRUE}
...
```{r chunk_chapter5_chunk24, echo=TRUE, eval=TRUE}
set.seed(543210)
...
```{r chunk_chapter5_chunk25, echo=TRUE, eval=TRUE}
hist(out, breaks = 50)
...
```{r chunk_chapter5_chunk26, echo=TRUE, eval=TRUE}
mean(out) - 0.5 # Should be ~0.
```

**Variance / standard deviation of the estimator**

```{r chunk_chapter5_chunk27, echo=TRUE, eval=TRUE}
sd(out)
```

To check p-values, we could run:

```{r chunk_chapter5_chunk28, echo=TRUE, eval=TRUE}
set.seed(12345)
...
To check the properties of other, possibly more complicated pipelines, we will typically use the same technique. I recommend doing this! Simulating often beats recommendations in the books!
