...
What do the random effects mean? The classical interpretation is that they are a structured error, so they are basically group residuals and are not of further importance. 

However, people increasingly see random effects also as a means to regularize fixed effect models. What does that mean? Note first that, conceptual, every random effect model corresponds to a fixed effect model. For a random intercept model, this would be 
...
Selecting with RE models

* Selecting on the fixed effect structure with AIC OK under the assumption that the RE structure doesn't change
* Selecting on the RE structure with lmerTest::ranova or simulated likelihood ratio tests
...
# causal assumptions
fit <- lmer(gpa ~ scale(occasion)*sex + as.numeric(job) + (1|student), data = gpa)
summary(fit)
...
# final model
fit <- lmer(gpa ~ scale(occasion)*sex + as.numeric(job) + (scale(occasion)|student), data = gpa)
summary(fit)
```
