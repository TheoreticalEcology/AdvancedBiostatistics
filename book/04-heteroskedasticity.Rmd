...
In this chapter, we check `lm()`{.R} and change the functional for our variance terms to improve the model.
...
With all the discussion about causality, model selection etc., we can re-visit the question of how to build an appropriate `lm()`{.R}. My basic modelling strategy for a causal `lm()`{.R} analysis is the following:
...
2. Use the gls function with the untransformed response to make the variance dependent on Solar.R. Hint: Read in `corClasses`.{R} and decide how to model this.
...
Alternative and very useful `quantreg`.{R} package: 
...
The data is <a href="https://elearning.uni-regensburg.de/pluginfile.php/2365103/mod_book/chapter/48053/Bees.txt" target="_blank" rel="noopener">here</a> Read it in via (don't forget to adjust the paths!):

```{r chunk_chapter4_task_3, message=FALSE, warning=FALSE, eval = FALSE, purl=FALSE}
bees = read.table("/tmp/Bees.txt", header = T)
```

Perform the data analysis, according to the hypothesis above.
