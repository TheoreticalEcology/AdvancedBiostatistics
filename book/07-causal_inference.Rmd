...
In detail, there are a few tricks to correct confidence intervals for the bootstrap, which are implemented in the `boot`{.R} package. Here is how you would do a boostrap with the boot package. The trick here is to implement the function f(), which must take the data as well as a selection of data points "k" (for example `c(1,3,4,5,8,9)`{.R}, or `1:20`{.R}, etc.) as input, and calculate the desired statistics. 
...
The same with the boot package. We need a statistics:
...
```{r chunk_chapter7_chunk16, echo=TRUE, eval=TRUE}
library(pbkrtest)
...
```{r chunk_chapter7_chunk17, echo=TRUE, eval=TRUE}
library(RLRsim)
library(lme4)
...
Structural equation models (SEMs) are models that are designed to estimate entire causal diagrams. For GLMs responses, you will currently have to estimate the DAG (directed aciclic graph) piece-wise, e.g. with <a href="https://cran.r-project.org/web/packages/piecewiseSEM/vignettes/piecewiseSEM.html" target="_blank" rel="noopener">https://cran.r-project.org/web/packages/piecewiseSEM/vignettes/piecewiseSEM.html</a>.

```{r chunk_chapter7_chunk20, echo=TRUE, eval=TRUE}
